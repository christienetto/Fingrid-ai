<script lang="ts">
    ...
    // Function to prevent the default navigation when clicking the image or vote buttons
    function handleZincClick(event) {
...
<a href="/post-timeline" class="w-full h-[500px] mx-auto flex font-inter py-6 cursor-pointer">
    ...
    <!-- Right side (image + vote buttons) -->
    <div class="flex flex-col relative right-0 w-3/5 h-full bg-zinc-400 rounded-r-xl overflow-hidden align-text-bottom border-2 border-zinc-600 border-l-0" on:click={handleZincClick}>
        <div class="h-full">
